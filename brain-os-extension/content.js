(function () {
  ...
  function sendMsg(payload, cb) {
    try {
      if (!chrome.runtime?.id) {
        console.warn(
          "[Brain OS] Extension context invalidated — reload the page",
        );
  ...
  const TOOLBAR_CSS = `
    ...
    * { box-sizing: border-box; margin: 0; padding: 0; }
    #tb {
      position: absolute; top: 0; left: 0; display: flex; align-items: center; gap: 1px; padding: 4px 6px;
      border-radius: 999px; background: rgba(14, 14, 16, 0.82); backdrop-filter: blur(20px) saturate(1.8);
      -webkit-backdrop-filter: blur(20px) saturate(1.8); border: 1px solid rgba(255, 255, 255, 0.13);
      box-shadow: 0 8px 32px rgba(0, 0, 0, 0.48), 0 2px 8px  rgba(0, 0, 0, 0.32), inset 0 1px 0 rgba(255, 255, 255, 0.08);
      opacity: 0; transform: translateY(5px) scale(0.96); transition: opacity  0.18s cubic-bezier(0.16, 1, 0.3, 1), transform 0.18s cubic-bezier(0.16, 1, 0.3, 1);
      pointer-events: none; will-change: transform, opacity; font-family: -apple-system, 'SF Pro Text', 'Inter', 'Segoe UI', system-ui, sans-serif; white-space: nowrap;
    }
    #tb.visible { opacity: 1; transform: translateY(0) scale(1); pointer-events: auto; }
    .sep { width: 1px; height: 14px; background: rgba(255, 255, 255, 0.1); margin: 0 3px; flex-shrink: 0; border-radius: 1px; }
    .code-badge { display: flex; align-items: center; gap: 3px; font-size: 9px; font-weight: 700; letter-spacing: 0.6px; text-transform: uppercase; color: rgba(163, 230, 53, 0.9); background: rgba(163, 230, 53, 0.1); border: 1px solid rgba(163, 230, 53, 0.2); border-radius: 999px; padding: 2px 7px 2px 5px; margin-right: 4px; }
    .btn { display: flex; align-items: center; gap: 5px; padding: 5px 10px; background: transparent; border: none; border-radius: 999px; color: rgba(255, 255, 255, 0.72); font-size: 11.5px; font-weight: 500; font-family: inherit; cursor: pointer; line-height: 1; outline: none; position: relative; overflow: hidden; transition: color 0.15s ease, background 0.15s ease; }
    .btn::before { content: ''; position: absolute; inset: 0; border-radius: 999px; background: radial-gradient(ellipse at 50% 50%, rgba(255,255,255,0.12) 0%, transparent 70%); opacity: 0; transition: opacity 0.2s ease; }
    .btn:hover { color: #ffffff; background: rgba(255, 255, 255, 0.09); }
    .btn:hover::before { opacity: 1; }
    .btn:active { transform: scale(0.96); }
    .btn.primary { color: rgba(255,255,255,0.92); background: rgba(255, 255, 255, 0.1); border: 1px solid rgba(255,255,255,0.14); }
    .btn.primary:hover { background: rgba(255, 255, 255, 0.16); color: #ffffff; }
    .btn.accent:hover { color: #10b981; background: rgba(16, 185, 129, 0.1); }
    .btn.accent:hover::before { background: radial-gradient(ellipse, rgba(16,185,129,0.15) 0%, transparent 70%); }
    .ic { display: flex; align-items: center; justify-content: center; flex-shrink: 0; opacity: 0.85; }
  ...
  function _buildToolbar() {
    ...
    const btns = [
      ...
      ...(code
        ? [{ action: "roast_code", icon: ICONS.flame, label: "Roast", cls: "" }]
        : []),
      null,
      {
        action: "magic_translate",
        icon: ICONS.globe,
        label: "Translate",
        cls: "",
      },
    ...
    for (const b of btns) {
      ...
      html += `<button class="btn ${b.cls}" data-action="${b.action}" title="${_actionTitle(b.action)}"><span class="ic">${b.icon}</span>${b.label ? `<span>${b.label}</span>` : ""}</button>`;
    }
  ...
  async function _runMagicTranslate(text) {
    ...
    try {
      ({ token, targetLanguage = "Hindi" } = await chrome.storage.local.get([
        "token",
        "targetLanguage",
      ]));
    } catch {
    ...
    wrapper.style.cssText =
      "transition: opacity 0.3s, filter 0.3s; filter: blur(0); opacity: 1;";
    try {
  ...
  window.addEventListener("brain:magic_translate", (e) =>
    _runMagicTranslate(e.detail.text),
  );
  ...
  function showToast(msg, type = "success") {
    ...
    toastEl.innerHTML = `<span style="color:${color};display:flex;align-items:center;flex-shrink:0">${icons[type] || ICONS.info}</span><span>${_esc(msg)}</span>`;
  ...
  function _buildPalette() {
    ...
    paletteEl.innerHTML = `
      <div id="__brain_palette">
        <div class="ph"><span class="ph-icon" style="color:#555;display:flex">${ICONS.search}</span><input id="__brain_pi" type="text" placeholder="Search your Second Brain…" autocomplete="off" spellcheck="false"><span class="ph-kbd">ESC</span></div>
        <div class="pr" id="__brain_pr"></div>
        <div class="pf"><span>↑↓ Navigate</span><span>↵ Select</span><span>ESC Close</span></div>
      </div>
    `;

    const style = document.createElement("style");
    style.textContent = `
      #__brain_palette_overlay { position:fixed;inset:0;z-index:2147483646; background:rgba(0,0,0,0.65); backdrop-filter:blur(6px);-webkit-backdrop-filter:blur(6px); display:flex;align-items:flex-start;justify-content:center; padding-top:18vh; opacity:0;transition:opacity 0.15s ease;pointer-events:none; font-family:-apple-system,'SF Pro Text','Inter',system-ui,sans-serif; }
      #__brain_palette_overlay.open{opacity:1;pointer-events:auto;}
      #__brain_palette { width:560px;max-width:calc(100vw - 32px); background:rgba(10,10,12,0.96); border:1px solid rgba(255,255,255,0.1); border-radius:14px; overflow:hidden; box-shadow:0 24px 80px rgba(0,0,0,0.8); transform:translateY(-8px) scale(0.98); transition:transform 0.15s cubic-bezier(0.16,1,0.3,1); }
      #__brain_palette_overlay.open #__brain_palette{transform:none;}
      .ph{display:flex;align-items:center;gap:10px;padding:13px 16px;border-bottom:1px solid rgba(255,255,255,0.06);} #__brain_pi{flex:1;background:transparent;border:none;outline:none;color:#e5e5e5;font-size:14px;font-family:inherit;} #__brain_pi::placeholder{color:#3a3a3a;} .ph-kbd{font-size:10px;color:#3a3a3a;background:#111;border:1px solid #222;border-radius:4px;padding:2px 7px;font-family:monospace;flex-shrink:0;} .pr{max-height:380px;overflow-y:auto;} .pr::-webkit-scrollbar{width:3px;} .pr::-webkit-scrollbar-thumb{background:#222;border-radius:3px;} .psl{padding:9px 16px 3px;font-size:10px;font-weight:700;letter-spacing:0.8px;text-transform:uppercase;color:#3a3a3a;} .pi{display:flex;align-items:center;gap:12px;padding:10px 16px;cursor:pointer;border-left:2px solid transparent;transition:background 0.1s,border-color 0.1s;} .pi:hover,.pi.act{background:rgba(255,255,255,0.04);border-left-color:#10b981;} .pi-ic{width:20px;display:flex;align-items:center;justify-content:center;color:#555;flex-shrink:0;} .pi-t{font-size:13px;font-weight:500;color:#e5e5e5;} .pi-s{font-size:11px;color:#555;margin-top:1px;} .pi-sc{font-size:10px;font-weight:700;color:#10b981;background:rgba(16,185,129,0.1);border-radius:3px;padding:1px 6px;flex-shrink:0;} .pe{text-align:center;padding:28px;color:#3a3a3a;font-size:13px;} .p-spin{display:flex;align-items:center;justify-content:center;padding:20px;gap:8px;color:#3a3a3a;font-size:12px;} .p-spin::before{content:'';width:14px;height:14px;border:1.5px solid #222;border-top-color:#10b981;border-radius:50%;animation:pspin 0.7s linear infinite;flex-shrink:0;} @keyframes pspin{to{transform:rotate(360deg);}} .pf{padding:8px 16px;border-top:1px solid rgba(255,255,255,0.05);display:flex;align-items:center;gap:16px;font-size:10.5px;color:#3a3a3a;}
    `;
    ...
    input.addEventListener("keydown", (e) => {
      ...
      if (e.key === "Enter") {
        ...
        if (a) _runPaletteAction(a.dataset.pa);
        return;
      }
    });
  ...
  function _markActive(items, idx) {
    items.forEach((el, i) => el.classList.toggle("act", i === idx));
  }
  ...
  function _buildDefaultResults(container) {
    ...
    if (isYouTube)
      html += `<div class="pi" data-pa="yt"><span class="pi-ic">${ICONS.youtube}</span><div><div class="pi-t">Sync YouTube Transcript</div><div class="pi-s">Extract &amp; save video knowledge</div></div></div>`;
    if (isGitHub)
      html += `<div class="pi" data-pa="gh"><span class="pi-ic">${ICONS.github}</span><div><div class="pi-t">Load GitHub README</div><div class="pi-s">Fetch repo context into Brain</div></div></div>`;
  ...
  async function _searchMems(query, container) {
    ...
    try {
      ...
      container.innerHTML = '<div class="pe">Extension error</div>';
      return;
    }
    if (!token) {
      container.innerHTML = '<div class="pe">Login to search memories</div>';
      return;
    }
    try {
      ...
      if (!mems.length) {
        container.innerHTML = '<div class="pe">No memories found</div>';
        return;
      }
      container.innerHTML =
        `<div class="psl">Memory Results (${mems.length})</div>` +
        mems
          .map(
            (
              m,
            ) => `<div class="pi" data-pa="load" data-text="${encodeURIComponent(m.content || "")}">
  ...
  function _closePalette() {
    ...
    if (inp) {
      inp.value = "";
      _buildDefaultResults(paletteEl.querySelector("#__brain_pr"));
    }
  }
  ...
  async function _runYTSync() {
    ...
    try {
      ...
      if (segs.length) {
        transcript = [...segs].map((s) => s.textContent.trim()).join(" ");
      }
      if (!transcript) {
      ...
      if (!transcript) {
        showToast("No captions found — enable CC first", "error");
        return;
      }
  ...
  if (isYouTube) {
    const ytInterval = setInterval(async () => {
      ...
      if (!controls) return;
      const hasCaptions = !!document
        .querySelector("script:not([src])")
        ?.textContent?.includes("captionTracks");
  ...
  async function _runGitHubAgent() {
    ...
    if (!m) {
      showToast("Open a GitHub repository page", "error");
      return;
    }
    const [, owner, repo] = m;
    ...
    try {
      ...
      if (apiRes.ok) {
        ...
        for (const sel of readmeSel) {
          ...
          if (el?.textContent?.length > 100) {
            readme = el.textContent.trim().substring(0, 4000);
            break;
          }
        }
      ...
      showToast(`GitHub: ${err.message}`, "error");
    }
  }
  ...
  function _findElement(selectorOrText) {
    ...
    // UPDATED: Now supports XPaths starting with parenthesis like (//a)[1]
    if (
    ...
    const interactables = [
      ...document.querySelectorAll(
        'button, a, input, textarea, select, [role="button"], [role="link"], [role="menuitem"], [role="option"], [tabindex], label',
      ),
    ];
    ...
    try {
      const attrSearch = document.querySelector(
        `[placeholder*="${CSS.escape(selectorOrText)}"], [aria-label*="${CSS.escape(selectorOrText)}"], [name*="${CSS.escape(selectorOrText)}"], [data-testid*="${CSS.escape(selectorOrText)}"]`,
      );
  ...
  // UPDATED: Now uses _retryFindElement instead of failing immediately
  async function _executeAction(action) {
    ...
    if (action.type === "select") {
      ...
      if (!el || el.tagName !== "SELECT")
        return { ok: false, error: `Select not found: ${action.selector}` };
    ...
    if (action.type === "read") {
      const el = (await _retryFindElement(action.selector)) || document.body;
      return { ok: true, content: el?.textContent?.trim().substring(0, 2000) };
    }
